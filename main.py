...
def main():
    ...
    label_similarity = load_files(model_cfg['DATASET']['SIM_GT'])

    # ------------ Construct Dataset Class ------------------------------------
    train_dset = Dset_VG(model_cfg, sg_train, label_similarity, vocab_glove, vocab2idx, 'train')
    train_dloader = DataLoader(train_dset, batch_size=model_cfg['MODEL']['BATCH_SIZE'], num_workers=args.num_workers, shuffle=True)

    test_dset = Dset_VG(model_cfg, sg_test, label_similarity, vocab_glove, vocab2idx, 'test')
